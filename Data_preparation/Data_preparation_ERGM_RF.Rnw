...
<<>>=
setwd('G:/My Drive/INFEWS_RC2_Work/Manuscripts/Trade paper/Communications Earth and Environment/Github_Aug2023')
@
...
<<>>=
# main input file
load('Main_input_Github.Rdata')
@

\section{Adjusting yield data}
<<>>=
Nyield_m = Nyield.kgkm[,167,45:54] # 167 index is for wheat and 45:54 is for 2005-2014
...
\subsection{Finding common countries}

<<>>=
c = 1
ii.tradeCo = tr_coName
ii.wiCo = tmpCoName
ii.yieldCo = unlist(FAOSTAT.CoName.FAO)[idx_noNA]
ii.cepii = rownames(distw_net)
ii.Co_1 = intersect(ii.tradeCo,ii.wiCo)
ii.Co_2 = intersect(ii.Co_1,ii.cepii)
ii.countries = intersect(ii.Co_2,ii.yieldCo)
@
...
\section{Preparing Data for ERGM model}
\subsection{Crop Yield Data}
<<>>=
# Correlation matrix with pearson correlation
CM_pearson <- cor(Ydata_Dtrend, method = "pearson")
CM_pearson <- CM_pearson[ii.countries, ii.countries]
@


\subsection{Trade Data}
Average trade data for years 2004 to 2014
<<>>=
...
\subsection{Network of extreme weather stress}
...
<<>>=
# index 167 is for wheat; 45:54 is for 2005-2014
Prod_data = t(Nproduction.kg[,167,45:54])
...
summary(ergm(ntwrk_trade_Duw ~ 
                 ...
                 edgecov(CM_pearson,attrname='STS')+
...
\subsection{Weighted trade network}

<<>>=
# diagnonal of pearson correlation is set to NA as we don't want self loop
CM_pearson_m1 <- CM_pearson
diag(CM_pearson_m1) <- NA
...
\subsection{Unweighted trade network}
<<>>=
CM_pearson_m1 <- CM_pearson
diag(CM_pearson_m1) <- NA
